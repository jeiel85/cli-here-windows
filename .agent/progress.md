...
- Applied first-pass modern UI theme: shared color/style tokens, card-based layout, stronger visual hierarchy, and updated primary/danger action button styling while preserving existing bindings/flows.
- Reworked theme to dark-only styling and palette harmony based on `claude-usage-tray-windows`, including dark input/list visuals and clearer primary/secondary/danger action hierarchy.
